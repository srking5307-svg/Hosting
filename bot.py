...
async def toggle_bot_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    effect = EFFECT_PARTY if notify_text == LIVE_MESSAGE else EFFECT_THUMBS_DOWN

    async def send_one(uid):
        try:
            await context.bot.send_message(uid, notify_text, parse_mode=ParseMode.MARKDOWN, message_effect_id=effect)
        except Exception:
            pass
...
async def show_stock_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply_long_text(update.message, text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN, message_effect_id=EFFECT_HEART)
...
async def handle_admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if action == 'edit_points':
            ...
            if not rows:
                await update.message.reply_text(header + "Abhi tak kisi ne bhi redeem nahi kiya.", parse_mode=ParseMode.MARKDOWN, message_effect_id=EFFECT_HEART)
            else:
                # Telegram hard-caps messages at 4096 chars - with 200-300+
                # redemptions this list easily blows past that and reply_text
                # throws "message is too long", which was showing up as the
                # generic "try again later" error no matter how many times
                # it was retried. Chunk it into multiple messages instead.
                chunks = []
                current = header
                current += "*Redeemed by:*\n"
                for r in rows:
                ...
                if current:
                    chunks.append(current)
                for chunk in chunks:
                    await update.message.reply_text(chunk, parse_mode=ParseMode.MARKDOWN, message_effect_id=EFFECT_HEART)
