...
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, ChatMemberHandler, filters, ContextTypes, ApplicationHandlerStop
...
def init_db():
    ...
    try:
        ...
        pass
    try:
        c.execute("ALTER TABLE redeem_codes ADD COLUMN protect_delivery INTEGER DEFAULT 1")
    except sqlite3.OperationalError:
        pass
...
def generate_receipt_image(item_name, price, buyer_label, purchase_date, purchase_id=None):
    """Draws a large, crisp, branded receipt card with Pillow. PORTRAIT
    format on purpose - a landscape (wide) image gets squashed into a short
    strip in Telegram's mobile chat preview, which is what made the earlier
    version look tiny even at higher pixel resolution. Portrait uses the
    chat bubble's height too, so it displays much bigger by default, and
    still renders at 2x supersampled resolution so it stays sharp when
    zoomed in on top of that."""
    from PIL import Image, ImageDraw, ImageFont
    import io

    SCALE = 2
    W, H = 1080 * SCALE, 1500 * SCALE
    gold = (255, 200, 60)
    ...
    margin = 50 * SCALE
    draw.rounded_rectangle([margin, margin, W - margin, H - margin], radius=36 * SCALE, outline=gold, width=6 * SCALE)
    draw.rounded_rectangle([margin + 16 * SCALE, margin + 16 * SCALE, W - margin - 16 * SCALE, H - margin - 16 * SCALE], radius=28 * SCALE, outline=(120, 90, 40), width=3 * SCALE)

    # Brand header - big and unmistakable
    y = margin + 70 * SCALE
    centered_text(y, "♛ SR KING ♛", font(70, True), gold_bright)
    y += 100 * SCALE
    centered_text(y, "GUEST ID STORE BOT", font(38, True), (255, 255, 255))
    y += 70 * SCALE
    centered_text(y, "✓ PURCHASE RECEIPT", font(42, True), gold)
    y += 90 * SCALE

    draw.line([(margin + 60 * SCALE, y), (W - margin - 60 * SCALE, y)], fill=gold, width=3 * SCALE)
    y += 70 * SCALE

    # Details block - large stacked label-over-value pairs, not side-by-side,
    # so long item names never get cramped or truncated
    content_w = W - 2 * (margin + 90 * SCALE)
    rows = [
        ("ITEM", item_name),
    ...
    for label, value in rows:
        draw.text((label_x, y), label, font=font(26, True), fill=(190, 160, 210))
        y += 44 * SCALE
        value_lines = wrap_text(str(value), font(48, True), content_w)
        for vline in value_lines:
            draw.text((label_x, y), vline, font=font(48, True), fill=(255, 255, 255))
            y += 62 * SCALE
        y += 30 * SCALE

    y += 10 * SCALE
    draw.line([(margin + 60 * SCALE, y), (W - margin - 60 * SCALE, y)], fill=(120, 90, 40), width=3 * SCALE)
    y += 70 * SCALE

    centered_text(y, "★ THANK YOU FOR SHOPPING ★", font(36, True), gold_bright)
    y += 60 * SCALE
    centered_text(y, "WITH SR KING!", font(36, True), gold_bright)
    y += 80 * SCALE
    centered_text(y, "Fast  •  Trusted  •  Instant Delivery", font(28), (210, 200, 220))

    img = img.resize((1080, 1500), Image.LANCZOS)
...
def build_user_actions_keyboard(target_user):
    ...
    ban_btn = (
        InlineKeyboardButton("✅ Unban", callback_data=f"uact_unban_{uid}", style="success") if target_user.get('banned')
        else InlineKeyboardButton("🚫 Ban", callback_data=f"uact_ban_{uid}", style="danger")
    )
    ghost_btn = (
        InlineKeyboardButton("👁 Un-Ghost", callback_data=f"uact_unghost_{uid}", style="primary") if target_user.get('shadow_banned')
        else InlineKeyboardButton("🕶 Ghost Ban", callback_data=f"uact_ghost_{uid}", style="danger")
    )
    return InlineKeyboardMarkup([
        [ban_btn, ghost_btn],
        [InlineKeyboardButton("➕ Add Credits", callback_data=f"uact_addcred_{uid}", style="success"),
         InlineKeyboardButton("➖ Deduct Credits", callback_data=f"uact_subcred_{uid}", style="danger")],
        [InlineKeyboardButton("🎁 Gift Bundle", callback_data=f"uact_gift_{uid}", style="success")]
    ])
...
async def user_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sub_action == "ban":
        ...
        keyboard = []
        for item in items:
            keyboard.append([InlineKeyboardButton(f"{item['item_name']} ({item['available']} left)", callback_data=f"giftitem_{target_id}_{item['item_id']}", style="primary")])
...
async def gift_item_confirm_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    confirm_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Yes, Gift It", callback_data=f"giftconfirm_{target_id}_{item_id}", style="success"),
         InlineKeyboardButton("❌ No", callback_data="giftconfirm_cancel", style="danger")]
    ])
    await query.message.reply_text(
...
async def gift_item_finalize(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            ...
            + format_credentials_block(assigned['item_name'], assigned['guest_uid'], assigned['guest_pass'], assigned.get('timestamp')),
...
MAX_CREDIT_AMOUNT = 1_000_000_000  # sane ceiling - SQLite INTEGER tops out at 2^63-1;
# a fat-fingered/huge typed amount was crashing with OverflowError instead of
# just being rejected with a normal error message.

def add_credits(user_id, amount):
    if not isinstance(amount, int) or abs(amount) > MAX_CREDIT_AMOUNT:
        print(f"⚠️ add_credits rejected out-of-range amount: {amount}")
        return False
    conn = get_db()
    c = conn.cursor()
...
def deduct_credits(user_id, amount):
    if not isinstance(amount, int) or abs(amount) > MAX_CREDIT_AMOUNT:
        print(f"⚠️ deduct_credits rejected out-of-range amount: {amount}")
        return False
    conn = get_db()
    c = conn.cursor()
...
async def reveal_credentials_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.edit_message_text(
            ...
            + format_credentials_block(purchase['item_name'], purchase['guest_uid'], purchase['guest_pass'], purchase.get('purchase_date')),
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception:
        pass
...
async def route_item_purchase_notification(context: ContextTypes.DEFAULT_TYPE, item_id, item_name, buyer_id):
    ...
    buyer_label = f"@{escape_md(buyer['username'])}" if buyer and buyer.get('username') and buyer['username'] != "NoUsername" else f"ID {buyer_id}"
    text = (
        f"🔥 *{escape_md(item_name)}* just got purchased!\n"
        f"👤 by {buyer_label}\n\n"
...
def create_redeem_code(credits, expiry_hours=None, max_uses=1, created_by=None, item_id=None, item_name=None, message=None, protect_delivery=True):
    ...
    protect_delivery controls whether the delivered credentials block
    forwarding/screenshots for THIS code specifically."""
    conn = get_db()
    c = conn.cursor()
    while True:
    ...
    c.execute("""INSERT INTO redeem_codes (code, credits, expiry, max_uses, used_count, created_by, created_at, active, item_id, item_name, message, protect_delivery)
                 VALUES (?, ?, ?, ?, 0, ?, ?, 1, ?, ?, ?, ?)""",
              (code, credits, expiry, max_uses, created_by, created_at, item_id, item_name, message, 1 if protect_delivery else 0))
    conn.commit()
    return {"code": code, "credits": credits, "expiry": expiry, "max_uses": max_uses, "item_id": item_id, "item_name": item_name, "message": message, "protect_delivery": protect_delivery}

def get_redeem_code(code):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT code, credits, expiry, max_uses, used_count, created_by, created_at, active, item_id, item_name, message, protect_delivery FROM redeem_codes WHERE code = ?", (code,))
    result = c.fetchone()
    if result:
        return {
            ...
            "protect_delivery": bool(result[11]) if result[11] is not None else True
        }
    return None
...
def redeem_code_for_user(code, user_id):
    ...
    if redeem["item_id"]:
        ...
        assigned["protect_delivery"] = redeem.get("protect_delivery", True)
...
def format_credentials_block(item_name, guest_uid, guest_pass, timestamp=None):
    """Guest ID and Password each get their OWN tap-to-copy field (single
    backticks around just that value) instead of one big JSON code block -
    tapping the whole JSON block used to copy everything at once with no
    way to grab just the ID or just the password. This works together with
    protect_content=True: copying text is a normal reading action Telegram
    always allows, it's forwarding/saving/screenshotting that gets blocked.

    Legacy Markdown breaks the ENTIRE message if any unescaped _ * ` [
    appears anywhere in it - item names commonly contain underscores
    (DARK_DESIRE_BUNDLE etc.), which was silently corrupting every message
    that included one. item_name goes through escape_md; guest_uid/guest_pass
    sit inside single-backtick code spans where underscores/asterisks are
    already safe, but a literal backtick INSIDE one would prematurely close
    it, so that one character gets stripped defensively (there's no valid
    escape sequence inside a legacy-Markdown code span)."""
    safe_item_name = escape_md(item_name)
    safe_uid = str(guest_uid).replace('`', "'") if guest_uid is not None else ""
    safe_pass = str(guest_pass).replace('`', "'") if guest_pass is not None else ""
    ts_line = f"🕐 {escape_md(timestamp)}\n\n" if timestamp else ""
    return (
        f"🎯 Item: {safe_item_name}\n"
        f"{ts_line}"
        f"🆔 Guest ID:\n`{safe_uid}`\n\n"
        f"🔑 Password:\n`{safe_pass}`\n\n"
        f"👆 Tap ID/Password to copy"
    )
...
async def reply_long_text(message, text, **kwargs):
    """Telegram hard-caps a single message at 4096 characters. Any list that
    grows with the data (users, redemptions, inventory items) will
    eventually blow past that and reply_text() throws - which looked like a
    generic, permanently-repeating 'try again later' error no matter how
    many times it was retried, since the SAME oversized text kept getting
    rebuilt and resent. Split into multiple messages instead, breaking only
    at line boundaries so formatting/markdown never gets cut mid-entity."""
    if len(text) <= 3500:
        ...
        if user and user.id not in ADMIN_IDS:
            if is_banned(user.id):
                try:
                    if update.callback_query:
                        ...
                        await update.callback_query.message.reply_text(BANNED_MESSAGE, parse_mode=ParseMode.MARKDOWN)
                    elif update.effective_message:
                        await update.effective_message.reply_text(BANNED_MESSAGE, parse_mode=ParseMode.MARKDOWN)
                except:
                    pass
                return
            if not get_bot_enabled():
                try:
                    if update.callback_query:
                        await update.callback_query.answer()
                        await update.callback_query.message.reply_text(MAINTENANCE_MESSAGE, parse_mode=ParseMode.MARKDOWN)
                    elif update.effective_message:
                        await update.effective_message.reply_text(MAINTENANCE_MESSAGE, parse_mode=ParseMode.MARKDOWN)
                except:
                    pass
                return
...
async def item_code_privacy_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    admin_id = query.from_user.id
    if admin_id not in ADMIN_IDS:
        await query.answer()
    ...
    protect_delivery = query.data == "itemcodeprivacy_on"
    ...
    custom_message = context.user_data.get('pending_item_code_message')

    if not item_id:
        try:
            await query.edit_message_text("⚠️ Item lost, please start again.")
        except Exception:
            pass
        await restore_admin_keyboard(context.bot, admin_id)
        return

    redeem = create_redeem_code(0, expiry_hours if expiry_hours > 0 else None, max_uses, created_by=admin_id, item_id=item_id, item_name=item_name, message=custom_message, protect_delivery=protect_delivery)
    log_admin_action(admin_id, "create_item_code", redeem["code"], f"Item: {item_name}, Expiry hrs: {expiry_hours}, Max uses: {max_uses}, Privacy: {protect_delivery}")
    ...
    privacy_note = "🔒 Privacy: ON" if protect_delivery else "🔓 Privacy: OFF"
    try:
        await query.edit_message_text(
            f"✅ *Item Redeem Code Created!*\n\n"
            f"🎟 Code: `{redeem['code']}`\n"
            f"🎁 Item: {escape_md(item_name)}\n"
    ...
    context.user_data['pending_item_code_expiry_hours'] = None
    context.user_data['pending_item_code_max_uses'] = None
    context.user_data['pending_item_code_message'] = None
    await restore_admin_keyboard(context.bot, admin_id)
...
async def show_trending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = c.fetchall()
    if not rows:
        await update.message.reply_text("📈 *Abhi tak koi purchase nahi hua hai.*", parse_mode=ParseMode.MARKDOWN)
        return
    text = "📈 *Trending Items*\n\n"
    medals = ["🥇", "🥈", "🥉"]
...
async def handle_preorder_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user['credits'] < selected['price']:
            await update.message.reply_text("⚠️ *Insufficient credits for preorder!*", parse_mode=ParseMode.MARKDOWN)
            return
        confirm_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Yes, Preorder", callback_data=f"preorder_yes_{item_id}", style="success"),
             InlineKeyboardButton("❌ No", callback_data="preorder_no", style="danger")]
        ])
        await update.message.reply_text(
            f"⚠️ *Preorder Confirmation*\n\n"
            f"Item: {escape_md(selected['item_name'])}\n"
...
async def fulfill_preorders(context: ContextTypes.DEFAULT_TYPE, item_id):
    ...
    Assigning stock has to stay sequential (each assignment picks the next
    available unit from a shared, limited pool - parallelizing that could
    double-assign the same account), but sending the notifications out
    afterward doesn't need to wait on anything, so that part runs concurrently."""
    conn = get_db()
    c = conn.cursor()
    ...
    for pre_id, pre_user_id, item_name in pending:
        ...
        conn.commit()
        to_notify.append((pre_user_id, item_name, assigned))

    async def notify_one(pre_user_id, item_name, assigned):
        try:
            await context.bot.send_message(
                pre_user_id,
                f"🎉 *Aapka preorder fulfil ho gaya!*\n\n"
                + format_credentials_block(item_name, assigned['guest_uid'], assigned['guest_pass'], assigned.get('timestamp')),
                parse_mode=ParseMode.MARKDOWN, protect_content=True
            )
        except Exception:
            pass
    ...
    sem = asyncio.Semaphore(25)
    async def notify_bounded(pre_user_id, item_name, assigned):
        async with sem:
            await notify_one(pre_user_id, item_name, assigned)

    await asyncio.gather(*(notify_bounded(uid, name, a) for uid, name, a in to_notify), return_exceptions=True)

    for pre_user_id, item_name, assigned in to_notify:
...
async def handle_buy_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"🛒 *Confirm Purchase*\n\n"
            f"Item: {escape_md(selected_item['item_name'])}\n"
    ...
    try:
        ...
        if deduct_credits(user_id, price):
            ...
            if assigned_item:
                ...
                reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
                reveal_kb = InlineKeyboardMarkup([[InlineKeyboardButton("👁 Tap to View Credentials", callback_data=f"reveal_{assigned_item['purchase_id']}", style="primary")]])
                await update.message.reply_text(
                    f"✅ *Purchase Successful!*\n\n"
...
async def handle_redeem_deep_link(message, context: ContextTypes.DEFAULT_TYPE, user_id, code):
    ...
    if success and kind == "credits":
        ...
        await message.reply_text(
            f"✅ *Code Redeemed!*\n\n"
            + format_credentials_block(payload['item_name'], payload['guest_uid'], payload['guest_pass'], payload.get('timestamp')),
            parse_mode=ParseMode.MARKDOWN,
            protect_content=payload.get("protect_delivery", True)
        )
        if payload.get("message"):
            await message.reply_text(f"📩 *Message from Admin:*\n\n{escape_md(payload['message'])}", parse_mode=ParseMode.MARKDOWN)
        ...
        await message.reply_text(reason_text, parse_mode=ParseMode.MARKDOWN)
...
async def handle_admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔙 Back":
        ...
        await update.message.reply_text("✅ Receipt mode: TEXT")
        await show_admin_panel(update, context)
    elif text == "🖼 Image Mode":
        update_setting("receipt_mode", "image")
        await update.message.reply_text("✅ Receipt mode: IMAGE")
        await show_admin_panel(update, context)
...
async def toggle_bot_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_ids = [u[0] for u in c.fetchall() if u[0] not in ADMIN_IDS]
    notify_text = LIVE_MESSAGE if new_state else MAINTENANCE_MESSAGE

    async def send_one(uid):
        try:
            await context.bot.send_message(uid, notify_text, parse_mode=ParseMode.MARKDOWN)
        except Exception:
            pass

    sem = asyncio.Semaphore(25)
    async def send_bounded(uid):
        async with sem:
            await send_one(uid)
    await asyncio.gather(*(send_bounded(uid) for uid in user_ids), return_exceptions=True)
    sent = len(user_ids)
...
async def show_stock_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await update.message.reply_text("📦 *Inventory is empty!*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
        return
    ...
    await reply_long_text(update.message, text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
...
async def show_channels_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    for ch in channels:
        keyboard.append([InlineKeyboardButton(f"❌ Remove {ch}", callback_data=f"rmchannel_{ch}", style="danger")])
    if keyboard:
...
async def show_leaderboard_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🤝 Top Referrals", callback_data="lb_referrals", style="primary")],
        [InlineKeyboardButton("🛒 Top Buyers", callback_data="lb_buyers", style="primary")],
        [InlineKeyboardButton("💰 Top Credits", callback_data="lb_credits", style="primary")],
    ])
    await update.message.reply_text("📊 *Leaderboard* — kaunsa dekhna hai?", reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN)
...
async def show_item_routes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if routes:
        for r in routes:
            ...
            keyboard.append([InlineKeyboardButton(f"❌ Remove: {r['item_name']} → {r['channel_id']}", callback_data=f"rmroute_{r['id']}", style="danger")])
    else:
...
async def show_code_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not codes:
        await update.message.reply_text("📋 *No redeem codes created yet.*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
        return
...
async def handle_item_code_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        selected = next((i for i in items if i["item_id"] == item_id), None)
        if not selected:
            await update.message.reply_text("⚠️ *Invalid item!*", parse_mode=ParseMode.MARKDOWN)
            return
...
async def handle_admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('user_action') == 'redeem_code':
        ...
        if success and kind == "credits":
            ...
            await update.message.reply_text(reason_text, parse_mode=ParseMode.MARKDOWN)
    ...
    try:
        if action == 'edit_points':
            ...
            if credits_val <= 0 or max_uses <= 0 or expiry_hours < 0:
                await update.message.reply_text("⚠️ *Values must be positive numbers!*", parse_mode=ParseMode.MARKDOWN)
                return
            ...
            if amount > MAX_CREDIT_AMOUNT:
                await update.message.reply_text(f"⚠️ *Amount too large! Max allowed: {MAX_CREDIT_AMOUNT:,}*", parse_mode=ParseMode.MARKDOWN)
                return
            ...
            try:
                ...
                await update.message.reply_text("🚫 *This user has blocked the bot — message not delivered.*", parse_mode=ParseMode.MARKDOWN)
            except Exception as e:
            ...
            confirm_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Yes, Send", callback_data="broadcast_yes", style="success"),
                 InlineKeyboardButton("❌ No, Cancel", callback_data="broadcast_no", style="danger")]
            ])
            await update.message.reply_text(
                f"📢 Preview:\n\n{text}\n\n"
            ...
            if len(parts) < 1 or len(parts) > 2:
                await update.message.reply_text("⚠️ *Invalid format! Use:* `expiry_hours [max_uses]`", parse_mode=ParseMode.MARKDOWN)
                return
            ...
            if not item_id:
                ...
                return
            custom_message = None if text.strip().lower() in ('skip', '-') else text.strip()
            context.user_data['pending_item_code_message'] = custom_message
            context.user_data['admin_action'] = None

            privacy_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🔒 Privacy ON (no forward/screenshot)", callback_data="itemcodeprivacy_on", style="success")],
                [InlineKeyboardButton("🔓 Privacy OFF (normal message)", callback_data="itemcodeprivacy_off", style="danger")]
            ])
            await update.message.reply_text(
                f"🔒 *Delivery Privacy*\n\n"
                f"Jab ye code redeem hoga, uska credentials message forward/share/screenshot-block rahe (recommended) ya normal rahe?",
            ...
            try:
                ...
                redeem = get_redeem_code(code)
                if not redeem:
                    await update.message.reply_text(f"⚠️ Code '{code}' not found!")
                    return
                rows = get_code_redemptions(code)
                reward = f"Item: {redeem['item_name']}" if redeem.get("item_id") else f"{redeem['credits']} credits"
                header = f"Code: {code} — {reward}\nUsed: {redeem['used_count']}/{redeem['max_uses']}\n\n"

                if not rows:
                    await update.message.reply_text(header + "Abhi tak kisi ne bhi redeem nahi kiya.")
                else:
                    # Telegram hard-caps messages at 4096 chars - with 200-300+
                    # redemptions this list easily blows past that and reply_text
                    # throws "message is too long". Chunk into multiple plain-text
                    # messages (no markdown at all here - it's diagnostic data, not
                    # display copy, so there's no reason to risk a parse failure
                    # over a stray character in someone's name).
                    chunks = []
                    current = header + "Redeemed by:\n"
                    for r in rows:
                        name = r['first_name'] or r['username'] or str(r['user_id'])
                        uname = f"@{r['username']}" if r['username'] else "no username"
                        line = f"- {name} ({uname}) (ID: {r['user_id']}) - {r['timestamp']}\n"
                ...
                print(f"⚠️ view_code_users failed: {e}")
            ...
            if amount >= 500:
                ...
                confirm_kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ Yes, Confirm", callback_data="highvalue_yes", style="success"),
                     InlineKeyboardButton("❌ No", callback_data="highvalue_no", style="danger")]
                ])
            ...
            if action == 'quick_add_credits':
                ...
                await update.message.reply_text(f"✅ {amount} credits add ho gaye!")
            else:
                deduct_credits(target_id, amount)
                log_admin_action(user_id, "quick_sub_credits", str(target_id), f"-{amount}")
                await update.message.reply_text(f"✅ {amount} credits deduct ho gaye!")
            context.user_data['admin_action'] = None
            context.user_data['pending_target_id'] = None
            ...
            parts = text.split()
            if len(parts) != 2:
                await update.message.reply_text("⚠️ *Format:* `item_id @channel`", parse_mode=ParseMode.MARKDOWN)
                return
            item_id = int(parts[0])
            channel_id = parts[1]
            items = get_all_special_items()
            ...
            await update.message.reply_text(f"✅ *Route Added!*\n\n{escape_md(item_name)} → {escape_md(channel_id)}", parse_mode=ParseMode.MARKDOWN)
            context.user_data['admin_action'] = None
            await show_admin_panel(update, context)
            ...
            if text.strip().lower() == 'clear':
                ...
                await update.message.reply_text("✅ *Welcome images pool cleared!*", parse_mode=ParseMode.MARKDOWN)
            else:
...
async def handle_inventory_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📥 Upload Users DB":
        ...
        try:
            ...
            if not export_data:
                await update.message.reply_text("⚠️ *No users in the database yet!*", parse_mode=ParseMode.MARKDOWN)
                return

            import io as _io
            buf = _io.BytesIO(json.dumps(export_data, indent=4, ensure_ascii=False).encode("utf-8"))
            buf.name = "users_db_export.json"
            await update.message.reply_document(
                document=buf,
                filename="users_db_export.json",
                caption=f"Users Database Export - Total users: {len(export_data)}"
            )
            log_admin_action(user_id, "export_users_db", "users", f"Exported: {len(export_data)} users")
        except Exception as e:
            print(f"⚠️ Export Users DB failed: {e}")
        ...
        try:
            ...
            import io as _io
            buf = _io.BytesIO(json.dumps(export_data, indent=4, ensure_ascii=False).encode("utf-8"))
            buf.name = "remaining_inventory.json"
            await update.message.reply_document(
                document=buf,
                filename="remaining_inventory.json",
                caption=f"Remaining Accounts Export - Total accounts: {len(export_data)}"
            )
            log_admin_action(user_id, "export_inventory", "inventory", f"Exported: {len(export_data)} accounts")
        except Exception as e:
            print(f"⚠️ Export Remaining Accounts failed: {e}")
            await update.message.reply_text(f"⚠️ Export failed: {e}")
...
async def handle_users_db_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not isinstance(data, list):
            await update.message.reply_text("⚠️ *Invalid format! Expected a list of users.*", parse_mode=ParseMode.MARKDOWN)
            return
...
async def handle_welcome_image_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT COUNT(*) FROM welcome_images")
    count = c.fetchone()[0]
    await update.message.reply_text(f"✅ Image added! Pool mein ab {count} images hain. Aur bhejo ya `cancel`/`skip` bhejo.")
...
async def handle_how_to_use_video_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ *How To Use video set ho gaya!*", parse_mode=ParseMode.MARKDOWN)
    context.user_data['admin_action'] = None
    await show_admin_panel(update, context)
...
async def handle_restore_db_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✅ *Database restored successfully!*", parse_mode=ParseMode.MARKDOWN)
        context.user_data['admin_action'] = None
        await show_admin_panel(update, context)
    except Exception as e:
...
def get_disabled_groups():
    raw = get_setting("disabled_groups")
    if not raw:
        return []
    try:
        parsed = json.loads(raw)
...
def save_disabled_groups(groups):
    update_setting("disabled_groups", json.dumps(groups))

async def group_gatekeeper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Runs FIRST (registered in group=-1, before every normal handler) for
    every update. A channel's linked discussion group mirrors the channel's
    inline buttons (like Redeem Now), and people were mass-tapping/replying
    to them in the group causing spam. /botoff lets an admin kill ALL bot
    button/command activity in that specific group instantly; /boton
    reverses it. Doesn't touch the channel itself or any other group."""
    ...
    disabled = get_disabled_groups()

    if text:
        command = text.split('@')[0].split()[0] if text.split() else ""
        if command == "/botoff" and user_id in ADMIN_IDS:
            if chat.id not in disabled:
                disabled.append(chat.id)
                save_disabled_groups(disabled)
            try:
                await update.message.reply_text("🔇 Bot disabled in this group — buttons/commands won't respond anymore. Use /boton to re-enable.")
            except Exception:
                pass
            raise ApplicationHandlerStop
        if command == "/boton" and user_id in ADMIN_IDS:
            if chat.id in disabled:
                disabled.remove(chat.id)
                save_disabled_groups(disabled)
            try:
                await update.message.reply_text("🔊 Bot re-enabled in this group.")
            except Exception:
                pass
            raise ApplicationHandlerStop
...
def main():
    ...
    # Runs before every other handler (lower group number = higher priority).
    # Handles /botoff /boton and silently kills all bot activity in groups
    # that have been disabled - see group_gatekeeper's docstring.
    application.add_handler(MessageHandler(filters.ALL, group_gatekeeper), group=-1)
    application.add_handler(CallbackQueryHandler(group_gatekeeper), group=-1)
    ...
    application.add_handler(CallbackQueryHandler(item_code_privacy_choice, pattern="^itemcodeprivacy_(on|off)$"))
    ...
    # Explicit allow-list instead of ALL_TYPES - we never handle edited
    # messages, channel posts, polls, etc. Letting those through was exactly
    # why editing a text message crashed handle_admin_text_input: an edited
    # message arrives as update.edited_message with update.message = None,
    # and filters.TEXT still matched it (it checks effective_message, not
    # message specifically), so the handler ran and hit update.message.text
    # on None. Not receiving these update types at all is the real fix.
