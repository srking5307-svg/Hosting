...
def maintenance_gate(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if user and user.id not in ADMIN_IDS and not get_bot_enabled():
            try:
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Regex(r'^💰 Edit Prices$|^🔄 Edit Points$|^🎁 Edit Bonus$|^🎁 Gift Credits$|^🔻 Deduct Credits$|^👤 User Info$|^📦 Upload Inventory$|^📢 Broadcast$|^📊 Stats$|^🎟 Create Code$|^📋 List Codes$|^🔌 Toggle Bot Status$'), handle_admin_buttons))
