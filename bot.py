...
def create_redeem_code(credits, expiry_hours=None, max_uses=1, created_by=None):
    ...
    c.execute("""INSERT INTO redeem_codes (code, credits, expiry, max_uses, used_count, created_by, created_at, active)
                 VALUES (?, ?, ?, ?, 0, ?, ?, 1)""",
              (code, credits, expiry, max_uses, created_by, created_at))
    conn.commit()
    conn.close()
    return {"code": code, "credits": credits, "expiry": expiry, "max_uses": max_uses}

def get_redeem_code(code):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT code, credits, expiry, max_uses, used_count, created_by, created_at, active FROM redeem_codes WHERE code = ?", (code,))
    result = c.fetchone()
    conn.close()
    if result:
        return {
            ...
            "created_at": result[6], "active": result[7]
        }
    return None
...
def redeem_code_for_user(code, user_id):
    """Attempts to redeem a code for a user. Returns (success: bool, reason: str, credits: int)."""
    ...
    return True, "success", redeem["credits"]

def list_recent_codes(limit=10):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT code, credits, expiry, max_uses, used_count, active FROM redeem_codes ORDER BY created_at DESC LIMIT ?", (limit,))
    results = c.fetchall()
    conn.close()
    return [{"code": r[0], "credits": r[1], "expiry": r[2], "max_uses": r[3], "used_count": r[4], "active": r[5]} for r in results]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not existing_user:
        ...
        if not member:
            ...
            # once they finish joining, instead of the main menu.
            if deep_link_redeem:
...
async def check_joined(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if all_joined:
        ...
        # drop the user straight into the redeem prompt instead of stopping
        # at the main menu.
        if context.user_data.pop('pending_redeem', False):
            await send_redeem_prompt(query.message, context)
    else:
...
async def show_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [KeyboardButton("🎟 Create Code", style="success"), KeyboardButton("📋 List Codes", style="primary")],
...
async def handle_admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('user_action') == 'redeem_code':
        ...
        success, reason, credits_won = redeem_code_for_user(code, user_id)
        context.user_data['user_action'] = None
        ...
        if success:
            ...
            }.get(reason, "⚠️ *Code redeem nahi ho paya!*")
    ...
    try:
        if action == 'edit_points':
            ...
            redeem_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🎁 Redeem Now", url=f"https://t.me/{bot_username}?start=redeem", style="success")]
            ])
            await update.message.reply_text(
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Regex(r'^💰 Edit Prices$|^🔄 Edit Points$|^🎁 Edit Bonus$|^🎁 Gift Credits$|^🔻 Deduct Credits$|^👤 User Info$|^📦 Upload Inventory$|^📢 Broadcast$|^📊 Stats$|^🎟 Create Code$|^📋 List Codes$'), handle_admin_buttons))
