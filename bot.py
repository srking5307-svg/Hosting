...
# ==================== COLORFUL BUTTONS (Telegram API 9.4+) ====================
def get_colorful_keyboard(buttons):
    """
    buttons: list of [text, callback_data, style]
    ...
    for row in buttons:
        btn_row = []
        for btn in row:
            if len(btn) >= 3 and btn[2]:
                btn_row.append(
                    InlineKeyboardButton(
                        text=btn[0],
                        callback_data=btn[1],
                        style=btn[2]  # "primary", "success", "danger"
                    )
                )
            else:
                btn_row.append(InlineKeyboardButton(text=btn[0], callback_data=btn[1]))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not existing_user:
        ...
        if not member:
            buttons = []
            for channel in CHANNELS:
                buttons.append([[f"📢 Join {channel}", f"join_{channel.replace('@', '')}", "primary"]])
            buttons.append([["✅ I have joined", "check_joined", "success"]])
            reply_markup = get_colorful_keyboard(buttons)
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use inline keyboard with colors for main menu
    buttons = [
        [["🛒 Buy Items", "menu_buy", "primary"], ["💰 My Credits", "menu_credits", "success"]],
        [["🎁 Daily Bonus", "menu_bonus", "success"], ["📤 Referral", "menu_referral", "primary"]],
        [["🏆 Leaderboard", "menu_leaderboard", "primary"], ["❓ Help", "menu_help", "danger"]]
    ]
    if user_id in ADMIN_IDS:
        buttons.append([["⚙️ Admin Panel", "menu_admin", "danger"]])
...
async def show_main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = get_user(user_id)
    if not user:
        await query.message.reply_text("⚠️ Error! Please /start again.")
        return
...
async def handle_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_buy":
        await show_buy_items_callback(update, context)
    elif data == "menu_credits":
        await show_my_credits_callback(update, context)
...
async def show_buy_items_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    items = get_all_special_items()
    user = get_user(query.from_user.id)
    
    if not items:
        await query.edit_message_text("⚠️ *No items available for purchase!*", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    for item in items:
        if item["available"] > 0:
            ...
            buttons.append([[btn_text, f"buy_{item['item_id']}", "primary"]])
    buttons.append([["🔙 Back", "back_main", "danger"]])
...
async def handle_buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not selected_item or selected_item["available"] <= 0:
        await query.edit_message_text("⚠️ *This item is out of stock!*", parse_mode=ParseMode.MARKDOWN)
        return
    
    user = get_user(user_id)
    if user["credits"] < selected_item["price"]:
        buttons = [[["🔙 Back", "back_main", "danger"]]]
        reply_markup = get_colorful_keyboard(buttons)
        await query.edit_message_text(
    ...
    # Show confirmation
    buttons = [
        [[f"✅ Confirm {selected_item['item_name']}", f"confirm_{item_id}", "success"]],
        [["❌ Cancel", "back_main", "danger"]]
    ]
    reply_markup = get_colorful_keyboard(buttons)
    await query.edit_message_text(
        f"🛒 *Confirm Purchase*\n\n"
...
async def handle_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if available <= 0:
        buttons = [[["🔙 Back", "back_main", "danger"]]]
        reply_markup = get_colorful_keyboard(buttons)
        await query.edit_message_text("⚠️ *Item out of stock!*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if user["credits"] < price:
        buttons = [[["🔙 Back", "back_main", "danger"]]]
        reply_markup = get_colorful_keyboard(buttons)
        await query.edit_message_text("⚠️ *Insufficient credits!*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if deduct_credits(user_id, price):
        ...
        if assigned_item:
            ...
            buttons = [[["🛒 Buy More", "menu_buy", "primary"], ["🏠 Home", "back_main", "success"]]]
            reply_markup = get_colorful_keyboard(buttons)
            await query.edit_message_text(
            ...
            buttons = [[["🔙 Back", "back_main", "danger"]]]
            reply_markup = get_colorful_keyboard(buttons)
            await query.edit_message_text("⚠️ *Error assigning item! Please contact admin.*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
    else:
        buttons = [[["🔙 Back", "back_main", "danger"]]]
        reply_markup = get_colorful_keyboard(buttons)
        await query.edit_message_text("⚠️ *Error processing purchase!*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)

async def show_my_credits_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    user = get_user(user_id)
    
    if not user:
        await query.edit_message_text("⚠️ Error!", parse_mode=ParseMode.MARKDOWN)
        return
...
async def show_admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await query.edit_message_text("⚠️ *Unauthorized!*", parse_mode=ParseMode.MARKDOWN)
        return
    
    buttons = [
        [["💰 Edit Prices", "admin_prices", "primary"], ["🔄 Edit Points", "admin_points", "success"]],
        [["🎁 Edit Bonus", "admin_bonus", "danger"], ["🎁 Gift Credits", "admin_gift", "success"]],
        [["🔻 Deduct Credits", "admin_deduct", "danger"], ["👤 User Info", "admin_userinfo", "primary"]],
        [["📦 Upload Inventory", "admin_upload", "primary"], ["📢 Broadcast", "admin_broadcast", "success"]],
        [["📊 Stats", "admin_stats", "primary"], ["🔙 Back", "back_main", "danger"]]
    ]
    reply_markup = get_colorful_keyboard(buttons)
    await query.edit_message_text(
        f"🔧 *Admin Panel*\n\n"
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_prices":
        await show_edit_prices_callback(update, context)
...
async def show_edit_prices_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    items = get_all_special_items()
    
    buttons = []
    for item in items:
        buttons.append([[f"{item['item_name']} - {item['price']}💰", f"price_{item['item_id']}", "primary"]])
    buttons.append([["🔙 Back", "admin_panel_back", "danger"]])
...
async def handle_price_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    item_id = int(query.data.split("_")[1])
    context.user_data['admin_action'] = f"price_{item_id}"
    await query.edit_message_text(
        f"💰 *Enter new price*\n\n"
...
async def show_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [[["🔙 Back", "admin_panel_back", "danger"]]]
    reply_markup = get_colorful_keyboard(buttons)
    await query.edit_message_text(
        f"📊 *Bot Statistics*\n\n"
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_price_callback, pattern="^price_"))
