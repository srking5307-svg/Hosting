...
MAIN_MENU, BUY_ITEMS, CONFIRM_PURCHASE, ADMIN_PANEL, ADMIN_EDIT_PRICE, ADMIN_EDIT_POINTS, ADMIN_EDIT_BONUS, ADMIN_GIFT, ADMIN_DEDUCT, ADMIN_USER_INFO, ADMIN_BROADCAST, ADMIN_UPLOAD = range(12)
...
def init_db():
    ...
    # Insert default prices for special items
    special_items = [
...
def get_inventory_count(item_id: int) -> int:
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def assign_inventory_to_user(user_id: int, item_id: int) -> Optional[Dict]:
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, item_id, item_name, guest_uid, guest_pass, timestamp FROM inventory WHERE item_id = ? AND sold = 0 LIMIT 1", (item_id,))
    result = c.fetchone()
...
def check_daily_bonus(user_id: int) -> Tuple[bool, str]:
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def add_inventory_bulk(items: List[Dict]) -> Dict:
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    added = 0
    skipped = 0
...
def get_all_special_items() -> List[Dict]:
    special_items = [
        ...
        for admin_id in ADMIN_IDS:
            try:
                ...
                msg = (
                    ...
                    f"📌 Type: {ref_text}\n"
                    f"📊 Points Added: {get_setting('refer_points') if referrer_id else '0'}\n"
                    f"🏆 Total Referrals: {user_data['total_referrals'] if referrer_id else '0'}"
                )
...
async def show_buy_items(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for item in items:
        if item["available"] > 0:
            btn_text = f"{item['item_name']} - {item['price']}💰 ({item['available']} left)"
            keyboard.append([KeyboardButton(f"{btn_text}||{item['item_id']}")])
...
async def handle_buy_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Confirm purchase
        keyboard = [
            [KeyboardButton(f"🟢 Confirm Purchase {selected_item['item_name']}||{item_id}")],
...
async def handle_confirm_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Deduct credits and assign item
...
async def show_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not top_referrers:
        await update.message.reply_text("📊 *Leaderboard*\n\nNo referrals yet! Be the first!", parse_mode=ParseMode.MARKDOWN)
        await show_main_menu(update, context)
        return
...
async def show_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [KeyboardButton("🔵 Upload Inventory"), KeyboardButton("🟢 Broadcast")],
        [KeyboardButton("🔴 Stats"), KeyboardButton("🔙 Back")]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text(
        f"🔧 *Admin Panel*\n\n"
...
async def handle_admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔙 Back":
        ...
        await show_edit_points(update, context)
    elif text == "🔴 Edit Bonus":
        await show_edit_bonus(update, context)
    elif text == "🔵 Gift Credits":
        await show_gift_credits(update, context)
    elif text == "🔴 Deduct Credits":
        await show_deduct_credits(update, context)
    elif text == "🟢 User Info":
        await show_user_info_prompt(update, context)
    elif text == "🔵 Upload Inventory":
        await show_upload_inventory(update, context)
    elif text == "🟢 Broadcast":
...
async def show_edit_prices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    for item in items:
        keyboard.append([KeyboardButton(f"{item['item_name']} - Current: {item['price']}💰||price_{item['item_id']}")])
...
async def handle_price_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        price = int(text)
        if price < 0:
            await update.message.reply_text("⚠️ *Price cannot be negative!*", parse_mode=ParseMode.MARKDOWN)
            return
        
        key = context.user_data.get('admin_action')
        if key and key.startswith("price_"):
            update_setting(key, str(price))
            log_admin_action(update.effective_user.id, "edit_price", key, f"New price: {price}")
            await update.message.reply_text(f"✅ *Price updated successfully!*\n\nNew price: {price}💰", parse_mode=ParseMode.MARKDOWN)
        else:
            await update.message.reply_text("⚠️ *Invalid action!*", parse_mode=ParseMode.MARKDOWN)
...
async def show_edit_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    current = get_setting("refer_points") or "5"
    await update.message.reply_text(
        f"🟢 *Edit Referral Points*\n\n"
        f"Current Points per Referral: {current}\n\n"
...
async def show_edit_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    current = get_setting("daily_bonus") or "10"
    await update.message.reply_text(
        f"🔴 *Edit Daily Bonus*\n\n"
        f"Current Daily Bonus: {current}\n\n"
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        value = int(text)
        if value < 0:
            await update.message.reply_text("⚠️ *Value cannot be negative!*", parse_mode=ParseMode.MARKDOWN)
            return
        
        action = context.user_data.get('admin_action')
        if action == 'edit_points':
            update_setting("refer_points", str(value))
            log_admin_action(update.effective_user.id, "edit_points", "refer_points", f"New value: {value}")
            ...
            log_admin_action(update.effective_user.id, "edit_bonus", "daily_bonus", f"New value: {value}")
            await update.message.reply_text(f"✅ *Daily bonus updated!*\n\nNew: {value} per day", parse_mode=ParseMode.MARKDOWN)
        else:
            await update.message.reply_text("⚠️ *Unknown action!*", parse_mode=ParseMode.MARKDOWN)
...
async def show_gift_credits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"🔵 *Gift Credits*\n\n"
        f"Format: `user_id amount`\n"
        f"Example: `123456789 50`\n\n"
        f"This will add credits to the user.\n"
        f"Type /cancel to cancel.",
        parse_mode=ParseMode.MARKDOWN
    )
    context.user_data['admin_action'] = 'gift'

async def show_deduct_credits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"🔴 *Deduct Credits*\n\n"
        f"Format: `user_id amount`\n"
        f"Example: `123456789 10`\n\n"
        f"This will remove credits from the user.\n"
        f"Type /cancel to cancel.",
        parse_mode=ParseMode.MARKDOWN
    )
    context.user_data['admin_action'] = 'deduct'

async def handle_gift_deduct(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if text == "/cancel":
        context.user_data['admin_action'] = None
        await show_admin_panel(update, context)
    ...
    try:
        ...
        if action == 'gift':
            ...
            log_admin_action(update.effective_user.id, "gift_credits", str(target_id), f"Amount: {amount}")
            await update.message.reply_text(
                f"✅ *Credits gifted!*\n\n"
            ...
            log_admin_action(update.effective_user.id, "deduct_credits", str(target_id), f"Amount: {amount}")
            await update.message.reply_text(
                f"✅ *Credits deducted!*\n\n"
                f"User: {target_user['first_name']} (@{target_user['username']})\n"
                f"Amount: {amount}💰\n"
        ...
        await update.message.reply_text("⚠️ *Please enter valid numbers!*", parse_mode=ParseMode.MARKDOWN)

async def show_user_info_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"🟢 *Get User Info*\n\n"
...
async def handle_user_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⚠️ *Please enter a valid user ID!*", parse_mode=ParseMode.MARKDOWN)

async def show_upload_inventory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"📦 *Upload Inventory*\n\n"
...
async def handle_inventory_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for item in filtered_items:
            if result['skipped'] == 0 or item not in filtered_items[-result['skipped']:]:
                item_counts[item.get("item_name")] += 1
...
async def handle_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_admin_action(update.effective_user.id, "broadcast", "all_users", f"Sent: {sent}, Failed: {failed}")
    await update.message.reply_text(
...
def main():
    ...
    application.add_handler(CommandHandler("cancel", lambda u, c: show_main_menu(u, c)))
    
    # Message handlers
    application.add_handler(MessageHandler(filters.Regex(r'^🟢 Buy Items$|^🔵 My Credits$|^🟢 Daily Bonus$|^🔵 Referral$|^🔵 Leaderboard$|^🔴 Help$|^🔴 Admin Panel$|^🔴 Cancel$|^🔙 Back$'), handle_menu_buttons))
    
    # Buy item selection handler
    application.add_handler(MessageHandler(filters.Regex(r'^.*\|\|\d+$'), handle_buy_selection))
    
    # Confirm purchase handler
    application.add_handler(MessageHandler(filters.Regex(r'^🟢 Confirm Purchase .*\|\|\d+$'), handle_confirm_purchase))
    
    # Admin panel handlers
    application.add_handler(MessageHandler(filters.Regex(r'^🔵 Edit Prices$|^🟢 Edit Points$|^🔴 Edit Bonus$|^🔵 Gift Credits$|^🔴 Deduct Credits$|^🟢 User Info$|^🔵 Upload Inventory$|^🟢 Broadcast$|^🔴 Stats$|^🔙 Back$'), handle_admin_buttons))
    
    # Admin price selection
    application.add_handler(MessageHandler(filters.Regex(r'^.* - Current: \d+💰\|\|price_\d+$'), handle_price_selection))
    
    # Admin input handlers
    application.add_handler(MessageHandler(filters.Regex(r'^\d+\s+\d+$'), handle_gift_deduct))
    application.add_handler(MessageHandler(filters.Regex(r'^\d+$'), handle_admin_input))
    
    # User info handler
    application.add_handler(MessageHandler(filters.Regex(r'^\d+$') & filters.Chat(ADMIN_IDS), handle_user_info))
    
    # Broadcast handler
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Chat(ADMIN_IDS), handle_broadcast))
    
    # Inventory upload handler
    application.add_handler(MessageHandler(filters.Document.ALL & filters.Chat(ADMIN_IDS), handle_inventory_upload))
