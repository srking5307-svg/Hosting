...
def generate_receipt_image(item_name, price, buyer_label, purchase_date):
    """Draws a simple styled receipt card with Pillow. No external API/cost -
    pure local image generation."""
    ...
    margin = 50
    draw.rounded_rectangle([margin, margin, W - margin, H - margin], radius=24, outline=(255, 200, 60), width=3)
    draw.text((margin + 30, margin + 25), "✅ PURCHASE RECEIPT", font=font(34, True), fill=(255, 220, 100))
    draw.line([(margin + 30, margin + 80), (W - margin - 30, margin + 80)], fill=(255, 200, 60), width=1)
    ...
    for label, value in rows:
        draw.text((margin + 30, y), f"{label}:", font=font(22, True), fill=(180, 180, 200))
        draw.text((margin + 220, y), str(value), font=font(22), fill=(255, 255, 255))
        y += 48

    draw.text((margin + 30, H - margin - 40), "Thank you for your purchase! 🎉", font=font(18), fill=(200, 200, 220))
...
async def send_purchase_receipt(context: ContextTypes.DEFAULT_TYPE, user_id, assigned_item, price):
    ...
    try:
        ...
        buf = generate_receipt_image(assigned_item['item_name'], price, buyer_label, now_ist().strftime("%Y-%m-%d %H:%M"))
        await context.bot.send_photo(user_id, photo=buf, caption="🧾 Your Purchase Receipt", protect_content=True)
    except Exception as e:
...
async def broadcast_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(admin_id, f"⚠️ Broadcast failed: {e}")
        except Exception:
            pass
...
async def handle_confirm_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not still_member:
            await update.message.reply_text(
                "⚠️ *Aapne humare channel(s) leave kar diye hain!*\n\n"
                "Purchase karne ke liye pehle sabhi channels dobara join karo.",
                parse_mode=ParseMode.MARKDOWN
            , message_effect_id=EFFECT_THUMBS_DOWN)
            await show_main_menu(update, context)
            return
